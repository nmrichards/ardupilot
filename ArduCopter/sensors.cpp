...
void Copter::read_rangefinder(void)
{
    ...
    static uint16_t dist_array[ARRAY_MAX];
    rangefinder.update();
    ...
    temp_alt = (float)temp_alt * MAX(0.707f, ahrs.get_rotation_body_to_ned().c.z);
     ...
     if (sizeof(dist_array) < ARRAY_MAX) {
       arr[sizeof(dist_array)] = temp_alt;
     } else {
       for(int i=1;i < sizeof(dist_array);i++) {
         arr[i-1] = arr[i]
       }
       arr[ARRAY_MAX] = temp_alt
     }
     if (CH13_OPT>=1500 && !takeoff_state.running) {
       for(int i=0;i < sizeof(dist_array);i++) {
           if(array[i] < temp_alt) {
             temp_alt=array[i];
           }
       }
